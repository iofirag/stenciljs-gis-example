import { PolylineShape, ShapeObject, ShapeType, PolylineShapeOptions, ShapeDefinition, Coordinate} from '../../../../../api-generated/wrapper/api-src';
...
export class PolylineShapeManager extends ShapeManagerBase {
	...
	// 	// Iterate coordinates
	// 	polyline.coordinates.forEach((coordinate: L.LatLngLiteral) => {
	...
	getCoordinateList(shapeObject: ShapeObject): Coordinate[] {
		const polyline: PolylineShape = shapeObject.shape as PolylineShape;
		let coordinates: any/* Coordinate[] */ = polyline.coordinates[0];
		return coordinates;
	}
	...
	shapeObjectToWkt(shapeObject: ShapeObject): string {
		const coordinate = this.getCoordinateList(shapeObject).map((item: Coordinate) => (`${item.lng} ${item.lat}`)).join(',');
	...
	shapeWktToObject(shapeWkt: string): ShapeObject {
		...
		const polylineObj: ShapeObject = {
			shape: </* PolylineShape */any>{ coordinates: coordinates },
