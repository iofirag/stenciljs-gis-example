// import { WktShape, ShapeDefinition, ShapeObject, PolygonShape, ShapeType, MultiPolygonShape, DrawBarOptions, DrawBarOptionsEdit, DrawBarOptionsDraw, ShapeData } from '../../../../../api-generated/wrapper/api-src';
...
import { WktShape } from '../../../../models/apiModels';

const textIconClassName: string = 'textLabelClass';

export default class DrawBarComp extends GisPluginBase {
	element: any;
	private drawableLayer: LPlus.FeatureGroup;
	...
	init() {
		...
		if (drawBar && drawBar.enable) {
			...
			if (this.element && this.drawableLayer) {
				this.context.map.on(LPlus.Draw.Event.CREATED, this.onDrawCreated); // on-click handler
				this.context.map.on(LPlus.Draw.Event.EDITED, this.onDrawEdited);
				this.context.map.on(LPlus.Draw.Event.DELETED, this.onDrawDeleted);
	...
	public getDrawLayer(): LPlus.FeatureGroup {
	...
	public export(): WktShape[] {
		...
		layers.forEach((layer: LPlus.Layer) => {
	...
	public import(wktShapesArr: Array<WktShape>) {
		...
		wktShapesArr.forEach((item: WktShape) => {
			...
			if (manager) {
				...
				const leafletObject: LPlus.Layer | LPlus.FeatureGroup = manager.addShapeToLayer(shapeDef, this.drawableLayer, { 
	...
	private drawEventHandler(drawEvent: any) {
		...
		const leafletObject: LPlus.Layer | LPlus.FeatureGroup = manager.addShapeToLayer(drawEvent.layer.shapeDef, this.drawableLayer, { 
			click: Utils.shapeOnClickHandler.bind(this, manager, this.context)
		});
		Utils.createBubble(leafletObject, BUBBLE_TYPE.TOOLTIP);
	}
	...
	private createElement(options?: DrawBarOptions): void {
		...
		if (!this.drawableLayer) {
			this.drawableLayer = new LPlus.FeatureGroup([]);
		}
		...
		this.element = new LPlus.Control.Draw(drawBarOptionsDev);
		this.context.map.addControl(this.element); // add this control to map
	}

	private createShapeDefFromDrawLayer(layer: LPlus.Layer, shapeType: ShapeType): ShapeDefinition {
	...
	private onDrawCreated(e: any): void {
		...
		e.layer = this.addShapeDefToLayer(e.layer, shapeDef) as LPlus.FeatureGroup;
	...
	private onDrawEdited(e: any): void {
		...
		e.layers.eachLayer((layer: LPlus.Layer) => {
	...
	private createWktFromEditableLayer(drawShapeLayer: LPlus.Layer): WktShape {
		const manager: ShapeManagerInterface = ShapeManagerRepository.getManagerByShapeDefLayer(drawShapeLayer);
	...
	private addShapeDefToLayer(layer: LPlus.FeatureGroup, shapeDef: ShapeDefinition): LPlus.FeatureGroup {
...
type DrawBarOptionsEdit_Dev = DrawBarOptionsEdit & {
	featureGroup: LPlus.FeatureGroup
};