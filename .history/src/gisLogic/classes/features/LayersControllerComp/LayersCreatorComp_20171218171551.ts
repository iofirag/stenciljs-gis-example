import { ShapeLayerDefinition, ClusterOptions, ShapeObjectOptions } from '../../../../../api-generated/wrapper/api-src';
...
export default class LayersCreatorComp {
	...
	    };
		...
		layerContainer.leafletClusterLayer = LayersCreatorComp.createClusterLayer(shapeLayerDef, context); // will be added later
		return layerContainer;
	}

	public static createHeatLayer(layer: ShapeLayerDefinition): LPlus.HeatLayer {
		let heatLayerGroup: LPlus.HeatLayer = null; 
		...
		function _createHeatLayerByCoordinates(heatData: Coordinate[]): LPlus.HeatLayer {
			return <LPlus.HeatLayer>new LPlus.HeatLayer(heatData, { minOpacity: MIN_OPACITY });
		}
	}

	public static createClusterLayer(layer: ShapeLayerDefinition, context: GisPluginContext): LPlus.MarkerClusterGroup {
		let clusterLayer: LPlus.MarkerClusterGroup = null; 
		...
		clusterLayer = LPlus.markerClusterGroup(clusterOptions_Dev);
		...
		for (const shapeDef of layer.shapes) {
			...
			if (manager) {
				...
				const leafletObject: LPlus.Layer | LPlus.FeatureGroup = manager.addShapeToLayer(shapeDef, clusterLayer, {
		...
		clusterLayer.on('clusterclick', (e: any) => {
			if (e.originalEvent.ctrlKey) {
				console.log('TBD select all shapes behind this cluster'); // TBD
			}
		});
		return clusterLayer;
	}
	...
	public static iconCreateFunction(cluster: LPlus.MarkerClusterGroup): any {
