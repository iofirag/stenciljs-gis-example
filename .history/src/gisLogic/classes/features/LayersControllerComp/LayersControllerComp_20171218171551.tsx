...
import { GisPluginBase, GisPluginContext, MapState, SelectionMode } from "../../pluginBase";
import {
  ShapeLayerDefinition,
  ShapeDefinition,
} from "../../../../../api-generated/wrapper/api-src";
...
import { ClusterHeat } from "../../../../../api-generated/wrapper/api-src/index";
...
export default class LayersControllerComp extends GisPluginBase {
  public element: any;
  ...
  public geoJsonToShapeDef(geoJson: LPlus.GeoJSON): ShapeDefinition {
    const shape: ShapeDefinition = {};
    // WKT
    if (geoJson.properties[EXPORT_SHAPE_FIELDS.shapeWkt]) {
      shape.shapeWkt = geoJson.properties[EXPORT_SHAPE_FIELDS.shapeWkt];
  }
    // Data
    if (geoJson.properties[EXPORT_SHAPE_FIELDS.shapeDataObj]) {
      shape.data = JSON.parse(geoJson.properties[EXPORT_SHAPE_FIELDS.shapeDataObj]);
    }
    // Options
    if (geoJson.properties[EXPORT_SHAPE_FIELDS.shapeOptionsObj]) {
      shape.options = JSON.parse(geoJson.properties[EXPORT_SHAPE_FIELDS.shapeOptionsObj]);
    }
    return shape;
  }
  public geoJsonListToShapeDefList(geoJsonList: LPlus.GeoJSON[]): ShapeDefinition[] {
    return geoJsonList.reduce((accumulator: ShapeDefinition[], geoJson: LPlus.GeoJSON) => {
  ...
  public onReadKml(fileNames: FileNames, onLoadEvent?: any): void {
    ...
    try {
      ...
      const geoJsonOfKml: LPlus.GeoJSON = togeojson.kml(kmlAsXml);

      if (geoJsonOfKml && geoJsonOfKml.features) {
        const geoJsonList: LPlus.GeoJSON[] = geoJsonOfKml.features;
  ...
  public onReadShp(fileNames: FileNames, onLoadEvent: any): void {
    ...
    if (!buffer) { throw 'Empty content'; }

    shp(buffer).then((geojsons: LPlus.GeoJSON[]) => {
      const geoJsonList: any = _.map(geojsons, (geojson: LPlus.GeoJSON) => geojson.features[0]);

      this.geoJsonToLeafletLayer(geoJsonList, fileNames);
    }).catch((ex: any) => {
      console.error('Shape File failed to load: ',ex);
    });
  }

  public addingDrawableLayerToLayerController(drawableLayer: LPlus.FeatureGroup) {
  ...
  private csv2geojsonCB(fileNames: FileNames, err: any, geoJsonOfCsv: LPlus.GeoJSON) {
    if (err) { throw 'Parse csv error' };

    if (geoJsonOfCsv && geoJsonOfCsv.features) {
      const prettifyGeoJsonList: LPlus.GeoJSON[] = [];

      // Prettify GeoJson properties
      const geoJsonList: LPlus.GeoJSON[] = geoJsonOfCsv.features;
      geoJsonList.forEach((geoJson: LPlus.GeoJSON, i: number) => {
  ...
  private geoJsonToLeafletLayer(geoJsonList: LPlus.GeoJSON[], fileNames: FileNames) {
  ...
  // private createShapesDefinitionListFromLeafletLayer(Layers: LPlus.Layer[]): ShapeDefinition[] {
  //   return _.reduce(Layers, (result: any, value: any) => {
  //     const manager: ShapeManagerInterface = ShapeManagerRepository.getManagerByLeafletLayer(value);
  //     const shapeObject: ShapeObject = manager.getShapeObjectFromDrawingLayer(value);
  //     let shapeWkt: string = manager.shapeObjectToWkt(shapeObject);

  //     if (shapeObject.type === ShapeType.MARKER) {
  //       // Check for our extended data, cause maybe it's actually a circle
  //       const shapeOptionalWkt: string = _.get(value,'feature.properties.shapeWkt');
  //       if (shapeOptionalWkt) {
  //         // Override point wkt for circle wkt
  //         shapeWkt = shapeOptionalWkt;
  //       }
  //     }
  //     // Check for extended data
  //     const data: ShapeData = _.cloneDeep(this.defaultShapeData);
  //     const paresedData: ShapeData = {};
  //     const shapeDataObj: string = _.get(value.feature.properties, "shapeDataObj");
  //     if (shapeDataObj) {
  //       const shapeDataObjFix: string = shapeDataObj.replace(/'/g, '"');
  //       _.assign(paresedData, JSON.parse(shapeDataObjFix));
  //     }
  //     _.merge(data, paresedData);

  //     // Check for extended options
  //     const options: ShapeObjectOptions = {};
  //     const shapeOptionsObj: string = _.get(value.feature.properties, "shapeOptionsObj");
  //     if (shapeOptionsObj) {
  //       const shapeOptionsObjFix: string = shapeOptionsObj.replace(/'/g, '"');
  //       _.assign(options, JSON.parse(shapeOptionsObjFix));
  //     }

  //     // Add new Shape-definition object
  //     result.push({ data, shapeWkt, options });
  ...
  private createElement(): void {
    ...
    this.styledLayerConroller = new LPlus.Control.StyledLayerControl(
      baseMaps,
      layers,
    ...
    const control = L.Control.extend({
      ...
      onAdd: (map: any) => {
        const div: HTMLElement = L.DomUtil.create('div', 'custom-layer-controller menu');
  ...
  private onOverlayChanged_updateSelections(e: any): void {
    ...
    // Iterate all shapes
    const leafletShapes: LPlus.Layer[] = e.layer.getLayers() || [];
    leafletShapes.forEach((leafletLayer: LPlus.Layer) => {
...
type LayerControllerOptions_Dev = {
  position: string;
  collapsed: boolean;
};
type FileNames = {
