...
import { ZOOM_TO_EXTEND_PADDING, FILE_TYPES, EXPORT_SHAPE_FIELDS, FILE_TYPES_ARRAY } from './statics';
import { GisPluginContext, MapState } from './pluginBase';
import { LPlus } from '../../declarations/LPlus';
import { ShapeDefinition, Bounds, Coordinate, MouseCoordinateOptions, ShapeData, IconOptions, ClusterHeat } from '../../models/apiModels';

// const canvg: any = require('canvg-browser');

// const tokml: any = require('tokml');
...
export default class Utils {
	 ...
	 */
	public static getVisibleLayers(mapState: MapState, map: LPlus.Map): LPlus.Layer[] {
		...
		// Drawable layers
		mapState.drawableLayers.forEach((drawLayer: LPlus.FeatureGroup) => {
	...
	public static getSelectedObjects(selectedLeafletObjects: { [key: string]: LPlus.Layer }): LPlus.Layer[] {
		// get all selected object
		return <LPlus.Layer[]>Object.values(selectedLeafletObjects);
	}
	...
	public static exportBlobFactory(
		...
		const relevantExportedLayers: LPlus.Layer[]   = Utils.getRelevantExportedLayers(selectedLeafletObjects, mapState, map);
		const geoJsonList:            LPlus.GeoJSON[] = Utils.shapeListToGeoJson(relevantExportedLayers);
	...
	public static createBubble(leafletObject: any, type: string): void {
		...
		if (leafletObject._layers) {
			// Multipolygon flow
			Object.values(leafletObject._layers).forEach((polygonItem: LPlus.Polygon) => {
	...
	public static selectedLeafletObjectHandler(context: GisPluginContext, leafletObject: LPlus.FeatureGroup | LPlus.Layer) {
	...
	public static zoomToExtend(mapState: MapState, map: LPlus.Map): boolean {
		let tempLayers: LPlus.FeatureGroup = new LPlus.FeatureGroup([]);

		const visibleShapes: LPlus.Layer[] = this.getVisibleLayers(mapState, map);
		visibleShapes.map((shape: LPlus.Layer) => {
		...
		if (tempLayers.getLayers().length === 0) {
			...
			const bounds: LPlus.LatLngBounds = tempLayers.getBounds() as LPlus.LatLngBounds;
	...
	public static zoomToExtendOnLayer(layerGroup: LPlus.LayerGroup | LPlus.FeatureGroup, map: L.Map): void {
		// Zoom to extend on new data
		let tempLayers: LPlus.FeatureGroup = new LPlus.FeatureGroup([]);

		const layerGroupLayers: LPlus.Layer[] = layerGroup.getLayers();
		layerGroupLayers.forEach((layer: LPlus.Layer) => {
		...
		// const bounds: LPlus.LatLngBounds = layer.getBounds() as LPlus.LatLngBounds;
		// const boundsMaxZoom: number = map.getBoundsZoom(bounds);
		// if (bounds && bounds._northEast && bounds._southWest && boundsMaxZoom) {
	...
	private static getBlobByType(type: FILE_TYPES, geoJsonList: LPlus.GeoJSON[], callback?: Function): Blob {
	...
	private static getRelevantExportedLayers(selectedLeafletObjects: { [key: string]: LPlus.Layer }, mapState: MapState, map: LPlus.Map): LPlus.Layer[] {
	...
	private static shapeListToGeoJson(visibleLayers: LPlus.Layer[]): LPlus.GeoJSON[] {
		let geoJsonList: LPlus.GeoJSON[] = [];

		visibleLayers.forEach((layer: LPlus.Layer) => {
			...
			let geoJson: LPlus.GeoJSON = layer.toGeoJSON();
	...
	public static uglifyCsvJson(geoJson: LPlus.GeoJSON): LPlus.GeoJSON {
		// Uglify shapeDataObj
	...
	public static pretifyCsvJson(geoJson: LPlus.GeoJSON): LPlus.GeoJSON {
	...
	private static exportToCSV(geoJsonList: LPlus.GeoJSON[]): string {
		const csv: string = Utils.createCsvFormatFromGeoJsonList(geoJsonList);
		return csv;
	}

	private static exportToSHP(geoJsonList: LPlus.GeoJSON[]): any {
	...
	private static exportToKML(geoJsonList: LPlus.GeoJSON[]): string {
		const kml: string = Utils.createKmlFormatFromLayers(geoJsonList);
		return kml;
	}

	private static createCsvFormatFromGeoJsonList(geoJsonList: LPlus.GeoJSON[]): string {
	...
	private static createKmlFormatFromLayers(geoJsonLayers: LPlus.GeoJSON[]): string {
		...
		geoJsonLayers.forEach((geoJson: LPlus.GeoJSON, i: number) => {
	...
	private static createKmlExtendedData(geoJson: LPlus.GeoJSON): string {
